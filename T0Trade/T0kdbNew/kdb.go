...
func (tb *tradeKdb) openDB() {
	...
	for iter.Next() {
		...
		logger.Info("load order form ldb %#v",order)
...
func (tb *tradeKdb) updateTab(o *Order) {
	...
	order := *o
	if order.Side == 1 {
		order.CumQty = -order.CumQty

		order.Orderqty = -abs(order.Orderqty)
	}
	go tb.send2Tab(FuncUpdate, ResponseTab, ord2resp(&order))
...
func (tb *tradeKdb) PushPosition(position Position) {
	kdbPosition := positionToKdb(position)
	go tb.send2Tab(PubAndUpdFunc, PositionTab, kdbPosition)

}

func (tb *tradeKdb) PushAsset(asset Asset) {
	kdbAsset := assetToKdb(asset)

	go tb.send2Tab(PubAndUpdFunc, AssetTab, kdbAsset)
}
...
func (tb *tradeKdb) start() {
	...
	tb.subscribe(EntrustTab, tb.sym)
