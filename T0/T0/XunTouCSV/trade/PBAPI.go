...
type PBApi struct {
	...
	assetFile    map[string]string
	positionFile map[string]string
	orderChan    chan *XunTouReq
	RspStrCh     chan string
	ErrStrCh     chan string
	AssetCh      chan string
	PositionCh   chan string
...
func NewPBAPI(accounts map[string]AccountInfo, orderDir, tempDir, resonseDir string) *PBApi {
	...
	pbApi := &PBApi{
		...
		AssetCh:      make(chan string, 10000),
		PositionCh:   make(chan string, 10000),
		...
		assetFile:    make(map[string]string, 100),
		positionFile: make(map[string]string, 100),
	}
	...
	for kdbAccount, account := range accounts {
		...
		//asset.32814516_7.20231113.csv
		pbApi.assetFile[kdbAccount] = fmt.Sprintf("%s/asset.%s_%s.%s.csv", resonseDir, account.Account, account.AccountType, day)
		//position.32814516_2.20231109.csv
		pbApi.positionFile[kdbAccount] = fmt.Sprintf("%s/position.%s_%s.%s.csv", resonseDir, account.Account, account.AccountType, day)
...
func (this *PBApi) start(isFull bool) {
	...
	readedFileMap := make(map[string]struct{}, 100)

	for k, _ := range this.responseFile {
		responseFile := this.responseFile[k]
		if _, ok := readedFileMap[responseFile]; ok {
			continue
		} else {
			readedFileMap[responseFile] = struct{}{}
		}
		go readCsv(responseFile, this.RspStrCh, 100)
	}
	for k, _ := range this.errorFile {
		...
		go readCsv(responseFile, this.ErrStrCh, 100)
	}

	for k, _ := range this.assetFile {
		responseFile := this.assetFile[k]
		if _, ok := readedFileMap[responseFile]; ok {
			continue
		} else {
			readedFileMap[responseFile] = struct{}{}
		}
		logger.Info("asset File :key:%v--->%v________________", k, responseFile)
		go readFullCsv(responseFile, this.AssetCh, 1000*3)
	}

	for k, _ := range this.positionFile {
		responseFile := this.positionFile[k]
		if _, ok := readedFileMap[responseFile]; ok {
			continue
		} else {
			readedFileMap[responseFile] = struct{}{}
		}
		logger.Info("positionFile File :%v________________", responseFile)
		go readFullCsv(responseFile, this.PositionCh, 1000*3)
	}
...
func readCsv(responseFile string, ch chan string, t time.Duration) {
	logger.Info("notify %s beggin !!!", responseFile)
	tk := time.NewTicker(t * time.Millisecond)
	...
	for scanner.Scan() {
		...
		//logger.Info("read msg::%s", line)
		ch <- line + "," + responseFile
	}
...
func readFullCsv(responseFile string, ch chan string, t time.Duration) {
	logger.Info("notify %s beggin !!!", responseFile)
	tk := time.NewTicker(t * time.Millisecond)
	for {
		select {
